...
const produitFavorieController={
    ...
    delete : async (req, res)=>{
        try{
            Model.produitFavorie.destroy({where:{id:req.params.id ,clientId:req.params.clientId }}).then((response)=>{
                if(response!==0){
                    return res.status(200).json({
                        success: true,
        ...
        try{
            Model.produitFavorie
              .findAll({
                ...
                attributes:["id"],
                include: [{ model: Model.produitlabrairie , attributes: { exclude: ["createdAt", "updatedAt","id"] }}],
              })
              .then((response) => {
                if(response!==null){
                    return res.status(200).json({
